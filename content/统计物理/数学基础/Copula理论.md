...
$$
\begin{aligned}
f_{X,Y}(x, y) = f_{U,V}(h_1(x, y), h_2(x, y)) \cdot |J|
\end{aligned}
$$
...
$$
\begin{aligned}
\frac{\partial v}{\partial x} = \frac{-\rho}{\sqrt{1-\rho^2}}, \quad \frac{\partial v}{\partial y} = \frac{1}{\sqrt{1-\rho^2}}
\end{aligned}
$$

$$
\begin{aligned}
J = (1 \cdot \frac{1}{\sqrt{1-\rho^2}}) - (0 \cdot \frac{-\rho}{\sqrt{1-\rho^2}}) = \frac{1}{\sqrt{1-\rho^2}}
\end{aligned}
$$
...
$$
\begin{aligned}
f_{U,V}(u, v) = \frac{1}{2\pi} e^{-\frac{u^2 + v^2}{2}}
\end{aligned}
$$
...
$$
\begin{aligned}
f_{X,Y}(x, y) = \frac{1}{2\pi \sqrt{1-\rho^2}} \exp\left( -\frac{1}{2} \left[ x^2 + \left( \frac{y - \rho x}{\sqrt{1-\rho^2}} \right)^2 \right] \right)
\end{aligned}
$$
...
$$
\begin{aligned}
\left[0,1\right]^d = \{ (p_1, p_2, \dots, p_d) : 0 \le p_i \le 1, \forall i = 1, \dots, p_d \}
\end{aligned}
$$
...
$$
\begin{aligned}
V_C(B) = C(y_1, y_2) - C(x_1, y_2) - C(y_1, x_2) + C(x_1, x_2)
\end{aligned}
$$
...
$$
\begin{aligned}
V_C(B) = \sum_{\mathbf{v} \in \text{vertices}(B)} \text{sgn}(\mathbf{v}) C(\mathbf{v})
\end{aligned}
$$
...
一个 $d$-维 Copula 是一个映射 $C: [0, 1]^d \rightarrow [0, 1]$，表现为**多元联合累积分布函数**（CDF），满足以下性质：

- **边界性（Grounded）：** 如果任何一个参数 $u_i = 0$，则 $C(\mathbf{u}) = 0$。

$$
\begin{aligned}
C(u_1, \dots, 0, \dots, u_d) = 0
\end{aligned}
$$
...
$$
\begin{aligned}
H(x, y) = F(x)F(y) \cdot [1 + \alpha (1 - F(x))(1 - F(y))]
\end{aligned}
$$
...
$$
\begin{aligned}
H(x, y) = (1 - e^{-x})(1 - e^{-y}) [1 + \alpha e^{-x} e^{-y}]
\end{aligned}
$$
...
$$
\begin{aligned}
H(x, \infty) = (1 - e^{-x})(1 - 0) [1 + \alpha e^{-x} \cdot 0] = 1 - e^{-x}
\end{aligned}
$$
...
$$
\begin{aligned}
|\rho| \le \frac{1}{4} = 0.25
\end{aligned}
$$

**结论：** 在旧的数学框架下，你费尽心机凑出的这个公式，**最高只能描述 0.25 的相关性**。

FGM 分布本质上是对独立性的一种**一阶线性扰动**。因为它只使用了边缘分布及其一阶补码的乘积，所以它无法捕捉变量之间深层或非线性的依赖关系，这导致了它只能描述极弱的相关性。
...
$$
\begin{aligned}
H(x, y) = \left[ (1 - e^{-x})^{-\theta} + (1 - e^{-y})^{-\theta} - 1 \right]^{-1/\theta}
\end{aligned}
$$
...
$$
\begin{aligned}
f(x, y) = \underbrace{c(u, v)}_{\text{依赖结构密度}} \times \underbrace{f_X(x) \cdot f_Y(y)}_{\text{边缘密度}}
\end{aligned}
$$
...
$$
\begin{aligned}
\lambda_L = \lim_{u \rightarrow 0^+} P(Y \le G^{-1}(u) | X \le F^{-1}(u)) = \lim_{u \rightarrow 0^+} \frac{C(u, u)}{u}
\end{aligned}
$$
...
$$
\begin{aligned}
F_X(x) = H(x, \infty) = (1 - e^{-x})(1 - 0)[1 + 0] = 1 - e^{-x}
\end{aligned}
$$
...
$$
\begin{aligned}
h(x, y) = e^{-x}e^{-y} \left[ 1 + \alpha (2e^{-x} - 1)(2e^{-y} - 1) \right]
\end{aligned}
$$
...
$$
\begin{aligned}
E[XY] = \int_{0}^{\infty} \int_{0}^{\infty} xy \cdot h(x, y) \, dx \, dy
\end{aligned}
$$
...
$$
\begin{aligned}
E[XY] = \int_{0}^{\infty} \int_{0}^{\infty} xy e^{-x}e^{-y} \left[ 1 + \alpha (2e^{-x} - 1)(2e^{-y} - 1) \right] dx dy
\end{aligned}
$$
...
$$
\begin{aligned}
\int_{0}^{\infty} x e^{-x} dx \cdot \int_{0}^{\infty} y e^{-y} dy = E[X]E[Y] = 1 \cdot 1 = 1
\end{aligned}
$$
...
$$
\begin{aligned}
\alpha \left( \int_{0}^{\infty} x e^{-x} (2e^{-x} - 1) dx \right) \left( \int_{0}^{\infty} y e^{-y} (2e^{-y} - 1) dy \right)
\end{aligned}
$$
...
$$
\begin{aligned}
I = \int_{0}^{\infty} x e^{-x} (2e^{-x} - 1) dx = \int_{0}^{\infty} (2x e^{-2x} - x e^{-x}) dx
\end{aligned}
$$
...
$$
\begin{aligned}
\alpha \cdot I \cdot I = \alpha \left(-\frac{1}{2}\right) \left(-\frac{1}{2}\right) = \frac{\alpha}{4}
\end{aligned}
$$
...
$$
\begin{aligned}
\text{Cov}(X, Y) = E[XY] - E[X]E[Y] = \left(1 + \frac{\alpha}{4}\right) - 1 = \frac{\alpha}{4}
\end{aligned}
$$
...
$$
\begin{aligned}
\rho = \frac{\text{Cov}(X, Y)}{\sigma_X \sigma_Y} = \frac{\alpha / 4}{1 \cdot 1}
\end{aligned}
$$
